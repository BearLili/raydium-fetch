...
const config_tokenArr =
  config?.amms?.filter((i) => i?.connector == "raydiumv4") || [];
...
async function fetchPoolKeys(connection, poolId, version = 4) {
  ...
  try {
    const accountInfoList =
      (await connection.getMultipleAccountsInfo(poolId)) || [];
...
async function compute(connection, poolKeysList, new_pool_keys_array) {
  try {
    ...
    const computeResultList = poolInfoList.map((poolInfo, idx) => {
      ...
      const amountToken = new TokenAmount(
      ...
      return {
        result: [
          Object.values(amountOut),
          { ...Object.values(amountIn), fee: null },
        ],
        tokenInfo,
        tokenJson,
        timestamp: Date.now(),
